...
\subsubsection{有限降序首次适应算法（Finite First Fit Decreasing，FFFD）设计}
	...
	%[1] Dósa, György (2007). "The Tight Bound of First Fit Decreasing Bin-Packing Algorithm Is FFD(I) ≤ 11/9OPT(I) + 6/9". Combinatorics, Algorithms, Probabilistic and Experimental Methodologies. ESCAPE. doi:10.1007/978-3-540-74450-4_1	
...
<<<<<<< HEAD
    ...
    \begin{figure}[!htbp]
        ...
        \begin{minipage}{0.48\linewidth}
            ...
            \caption{数据集A3（批次22板材26）}
        \end{minipage}
        \begin{minipage}{0.48\linewidth}
            \centering
            \includegraphics[width=.9\textwidth]{B4-Batch28-YMH-0218SD-Board16.png}
            \caption{数据集A4（批次28板材16）}
        \end{minipage}
    \end{figure}
=======
	
\subsubsection{启发式算法设计}	

	\textbf{算法思路}
	
	所建立的大规模混合整数线性规划模型，求解耗时长、传统方法难以求解，可用启发式算法在多项式时间内求得近似最优解。启发式算法对初值敏感，离可行域较远的初值很可能导致不收敛、找不到最优解。而FFFD算法计算得到的可行解，可作为启发式算法的较优初值，作为搜索的起点。

	

>>>>>>> 180e494f6b164510096a80eb5584ede85a8103c1
\subsection{算法及效果评价}
