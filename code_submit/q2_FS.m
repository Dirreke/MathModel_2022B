function [batches,ratio,num_plates] = q2_FS(data_ori,width,height,max_item_number,max_item_area)
...
while is_merge
    ...
    for k = 1:size(material_data, 1)
        ...
        for kk = 1:length(orders)
            ...
            tmp_num_1 = length(q1_FFFD_fun(tmp_data_1_2, width, height));
            ...
            for kkk = kk + 1:length(orders)
                ...
                tmp_num_2 = length(q1_FFFD_fun(tmp_data_2_2, width, height));
                % order_1+2 的材质k的排版数量
                tmp_data_12 = [tmp_data_1_2; tmp_data_2_2];
                tmp_num_12 = length(q1_FFFD_fun(tmp_data_12, width, height));
...
for k = 1:size(orders_unique, 1)
    [material_packs,material_packs_ratio,material_packs_num_plates] = q2_FFFD_fun(data, width, height, orders_unique(k));
