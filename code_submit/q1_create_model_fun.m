function model = q1_create_model_fun(width, height, data_ori)
...
for k = 1:n - 1
    ...
    for kk = k + 1:n
        ...
        if data_ori(kk, 4) ~= data_ori(k, 4) || data_ori(kk, 3) + data_ori(k, 3) > height
            index_3((k - 1) * n + kk) = 1;
        end
...
for k = 1:n - 1 % j
    A_5(k, n * k - n + k:n * k) = data_ori(k:n, 3)';
    A_5(k, n^2 + k:n:n^2 + k * n) = -data_ori(1:k, 3)';
end
...
for k = 1:n - 1 % k
    A_6(k, n^2 + n * k - n + k:n^2 + n * k) = data_ori(k:n, 4)';
    A_6(k, n^2 + n * k - n + k) = A_6(k, n^2 + n * k - n + k) - width;
end
...
for k = 1:n - 1 % l
    A_8(k, 2 * n^2 + n * k - n + k:2 * n^2 + n * k) = data_ori(k:n, 3)';
    A_8(k, 2 * n^2 + n * k - n + k) = A_8(k, 2 * n^2 + n * k - n + k) -height;
end
